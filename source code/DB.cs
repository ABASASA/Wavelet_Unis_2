...
namespace DataSetsSparsity
{
    class DB
    {
        ...
        //ASAFAB - 
        public static bool CheckOfEachChild(double[] hyperCoeff, double[] point, double[] mean)
        {
            double[] tmpPoint = new double[point.Count() + 1];
            double score = hyperCoeff[point.Count()];
            for (int index = 0; index < point.Count(); index ++)
            {
                score += hyperCoeff[index] * (point[index] - mean[index]);
            }
            return score > 0; 
        }
